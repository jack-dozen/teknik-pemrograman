...
public class Fulltime extends Employee implements Koperasi{
    private double gajiPokok, tunjanganLembur, tunjanganJabatan;
    private final double pinjamanKoperasi, tunjanganKomunikasi;
    ...
    public Fulltime(String nama, String jabatan, int tahunMulaiKerja, int jumlahAnak, int bulanSekarang, int tahunSekarang, int jumlahHariKerja, boolean isWeekend, int jamLembur, double pinjamanKoperasi){
        super(nama, jabatan, tahunMulaiKerja, jumlahAnak, bulanSekarang, tahunSekarang, jumlahHariKerja, isWeekend, jamLembur);
        this.pinjamanKoperasi = pinjamanKoperasi;
        this.tunjanganKomunikasi = 500000;
    }
    ...
    private double hitTunjanganJabatan(){
        if (tahunSekarang - tahunMulaiKerja >= 3){
            if(jabatan.compareTo(SM) == 0)
    ...
    private double hitTunjanganAnak(){
        return (jumlahAnak <= 3) ? (jumlahAnak * 500000) : (3 * 500000);
    }

    private double hitBonusProduktivitas(){
        return (8 * jumlahHariKerja) + jamLembur;
    }

    @Override
    public double getSalary(){
        return hitungGajiPokok() + hitTunjanganLembur() + hitTunjanganJabatan() + hitTunjanganAnak()
        + tunjanganKomunikasi + hitBonusProduktivitas() - pinjamanKoperasi;
    }
    ...
    public void printSlipGaji(){
        ...
        System.out.println("Total Tunjangan: Rp" + (hitTunjanganLembur() + hitTunjanganJabatan() + hitTunjanganAnak() + tunjanganKomunikasi));
        System.out.println("Total Pinjaman: Rp" + pinjamanKoperasi);
